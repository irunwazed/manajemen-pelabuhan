...
<style>
    ...
    <div class="">
        ...
        <form id="permohonan-1c" action="{{url($user.'/penyewaan-alat/permohonan-1c/filter')}}" method="get">
            ...
            <div class="w-full">
    <a href="{{url($user.'/penyewaan-alat/create-permohonan-1c')}}" class="text-white float-right bg-green-400 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-10 py-2.5 mr-2 mb-2 dark:bg-green-400 dark:hover:bg-green-400 focus:outline-none dark:focus:bg-green-400">Tambah</a>
  </div>
                        ...
                        <tr class="border-solid border-2 border-slate-800 hover:bg-slate-300">
                            ...
                            <td class="py-2 flex flex-wrap gap-1 justify-center">
                                @if(empty($item->tgl_noform_2c))
                                    <a href="{{ url($user.'/penyewaan-alat/edit-permohonan-1c').'/'.$item->pbau_alat_1c_id }}" class="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:focus:ring-yellow-900">Edit</a>
                                @else
                                    <a href="{{ url($user.'/penyewaan-alat/edit-permohonan-1c').'/'.$item->pbau_alat_1c_id }}" class="focus:outline-none text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-purple-600 dark:hover:bg-purple-700 dark:focus:ring-purple-900">View</a>
                                @endif
                            </td>
                        </tr>
                    @endforeach
