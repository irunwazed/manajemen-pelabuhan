...
@section('content')
    <div class="">
        <div class="text-2xl ">Aneka Usaha / Sewa Bunker / Detail Sewa Bunker</div>
        <hr class="border-b-2 border-black border-solid">
        <div class="font-bold text-2xl text-center pt-5">Entry Permohonan Bunker</div>
        <br>
        <div class="h-100 grid grid-cols-1 gap-5 content-center">
            <div>
                <table class="w-full">
                    ...
                    <tr class="text-start">
                        ...
                        <td class="py-1">
                            <div class="">
                                <select id="barang" onchange="getData(this.value)"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    name="perusahaan">
                                    @foreach ($data['perusahaan'] as $tuslag)
                                        <option value="{{ $tuslag->perusahaan_id }}">{{ $tuslag->perusahaan_id }} -
                                            {{ $tuslag->nama_perusahaan }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <tr class="text-start">
                        ...
                        <td>:</td>
                        <td class="py-1">
                            <input type="text" value="" id="alamat"
        ...
        <div class="h-40 grid grid-cols-1 gap-1 content-center">
            <div>
                <table class="w-full">
                    <tr class="text-start">
                        <td>Kode Kapal</td>
                        ...
                        <td class="py-1">
                            <div class="">
                                <select id="barang" onchange="getData(this.value)"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    name="kapal">
                                    @foreach ($data['kapal'] as $kapal)
                                        <option value="{{ $kapal->kode_kapal }}">{{ $kapal->kode_kapal }} -
                                            {{ $kapal->nama_kapal }}
                                        </option>
                                    @endforeach
                                </select>
        ...
        <div class="text-center pt-16 mt-16 pb-9">
            ...
            <a href="{{ url('admin/aneka-usaha/permohonan-sewa-bunker') }}"
                class="btn mt-6 bg-blue-600 text-blue-100 hover:bg-purple-600">Batal</a>
...
@section('script')
    <script>
        ...
        function getData(id) {
            $.ajax({
                type: 'POST',
                url: "{{ route('getPerusahaan', 'admin') }}",
                data: {
                    data: id,
                },
                success: function(response) {
                    console.log(response);
                    $("#alamat").val(response.perusahaan.alamat);
                    $("#npwp").val(response.perusahaan.npwp);
                }
            });
        }
    </script>
