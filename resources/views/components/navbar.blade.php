...
<ul class="menu flex">
  ...
  <li>
    <button class="py-2 px-4 mx-2 text-gray-500 flex relative active" id="{{ $navigation->display  }}" data-dropdown-toggle="{{ $navigation->display.'-bar' }}">
      <span class="mr-0"> {{ $navigation->display  }} </span>
    ...
    <div id="{{ $navigation->display.'-bar' }}" class="z-10 hidden font-normal overflow-y-scroll max-h-[500px] bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 dark:divide-gray-600">
      <ul class="py-2 text-sm text-gray-700 dark:text-gray-400" aria-labelledby="dropdownLargeButton">
      <!-- untuk list dropdown menu dari setiap parent sesuai dengan akses user yang login -->
          @foreach(@session()->get("submenu") as $sub)
          @if($navigation->id_menu===$sub->parent)
          <li>
            <a href="{{ url('/').'/'.session()->get("usergroup").''.$sub->slug }}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{{ $sub->display  }}</a>
          </li>
          @endif
          @endforeach
       <!--batas -->   
